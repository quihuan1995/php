<script>
	function buynow(){                                                // javascrip kiểm tra a onclick
		document.getElementById('buy-now').submit();                  // truy vấn form id
	}
</script>
<?php
...
?>
				...
				if(isset($_SESSION['cart']) && count($_SESSION['cart'])>0){ //kieem tra session gio hang
					if(isset($_POST['sbm'])){
							foreach($_POST['qtt'] as $prd_id=>$qtt){   //foreach upload số lượng giỏ hàng khi cập nhật
								$_SESSION['cart'][$prd_id]=$qtt;
							}
					}
					
					foreach($_SESSION['cart'] as $prd_id => $qtt){       //foreach giỏ hàng
						$arr_id[]= $prd_id;							   //cho 1 mảng id[]
					}
					
					$str_id = implode(',',$arr_id);						//cho 1 str_id và implode mảng id
				...
				?>
                    ...
                    <form method="post">
                    <?php
					$total_price_all=0;                               //cho tổng số tiền =0
                  		while($row = mysqli_fetch_array($query)){
							$total_price = $row['prd_price']* $_SESSION['cart'][$row['prd_id']];  //số tiền = giá * session giỏ hàng.prd_id
							$total_price_all += $total_price;                                     // tổng số tiền += số tiền
                    ?>
        ...
        <!--ex-->            <input name="qtt[<?php echo $row['prd_id']; ?>]" type="number" id="quantity" class="form-control form-blue quantity" value="<?php echo $_SESSION['cart'][$row['prd_id']]; ?>" min="1">   <!-- name:qtt mảng prd_id --> <!-- value: sesion giỏ hàng.prd_id -->
                        </div> 
       <!--ex-->        <div class="cart-price col-lg-3 col-md-3 col-sm-12"><b><?php echo $total_price; ?></b><a href="modules/cart/cart_del.php?prd_id=<?php echo $row['prd_id']; ?>">Xóa</a></div>    
                    </div>  
                  <?php
						}
					?>
 ...
 <!--ex-->               <div class="cart-price col-lg-3 col-md-3 col-sm-12"><b><?php echo $total_price_all; ?></b></div>
                    </div>
                    </form>
                         <?php
							}
						  ...
						  ?>
                          <div class="alert alert-danger">ko giỏ hàng</div>
                          <?php
							}
						  ?>
                </div>
                ...
                if(isset($_POST['name'])&&isset($_POST['phone'])&&isset($_POST['mail'])&&isset($_POST['add'])){
					...
					$str_body ='';                             //cho 1 biến strbody = rỗng
					$str_body .='
					<p>
					...
					$str_body .='<tr>
									...
									<td width="20%"><b><font color="#FF0000">'.$total_price_all.'</font></b></td>
										</tr>
									</table>';
